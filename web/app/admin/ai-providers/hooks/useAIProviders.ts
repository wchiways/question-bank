...
export function useAIProviders() {
  ...
  const updateProvider = useCallback(async (providerKey: string, values: any) => {
    try {
  ...
  const testProvider = useCallback(async (providerKey: string): Promise<TestResult> => {
    setTesting(prev => ({ ...prev, [providerKey]: true }));

    try {
      const res = await fetch(`/api/v1/admin/ai/providers/${providerKey}/test`, {
        method: 'POST',
      });
  ...
  const setDefaultProvider = useCallback(async (providerKey: string) => {
    try {
  ...
  const batchEnable = useCallback(async (providerKeys: string[]) => {
    try {
  ...
  const batchDisable = useCallback(async (providerKeys: string[]) => {
    try {
  ...
  const createProvider = useCallback(async (providerData: any) => {
    try {
      const res = await fetch('/api/v1/admin/ai/providers', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
